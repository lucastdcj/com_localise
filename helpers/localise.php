...
abstract class LocaliseHelper
{
	...
	public static function getFileId($path)
	{
		...
		if (!isset($fileIds))
		{
			...
			$query->from($db->nameQuote('#__localise_file'));
			$db->setQuery($query);
	...
	public static function getFileAssetId($path)
	{
		static $fileAssetIds = null;
		if (!isset($fileAssetIds))
		{
			$db = JFactory::getDBO();
			$query = $db->getQuery(true);
			$query->select('asset_id, path');
			$query->from($db->nameQuote('#__localise_language'));
			$db->setQuery($query);
			$fileAssetIds = $db->loadObjectList('path');
		}
		...
		if (JFile::exists($path) || preg_match('/.ini$/', $path))
		{
			if (!array_key_exists($path, $fileAssetIds))
			{
				...
				$table->path = $path;
				$table->store();
				$fileAssetIds[$path] = new JObject(array('asset_id' => $table->asset_id));
			}
			return $fileAssetIds[$path]->asset_id;
		}
		else
		{
			$asset_id = 0;
