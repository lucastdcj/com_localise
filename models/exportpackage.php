...
class LocaliseModelExportPackage extends JModelItem
{
	...
	public function getItem() 
	{
		...
		$path = JPATH_ROOT . "/media/com_localise/packages/$packageName.xml";
...
<extension type="file" version="1.6" method="upgrade">
	...
	<languages>';
		...
		{
			$files['manifest']['data'].= '
		<language tag="' . $tag . '">language/' . $tag . '/' . $tag . '.' . $keyName . '.ini</language>';
			$files['manifest']['data'].= '
		<language tag="' . $tag . '">language/' . $tag . '/' . $tag . '.' . $keyName . '.manage.ini</language>';
		}
		$files['manifest']['data'].= '
	</languages>
</extension>';
